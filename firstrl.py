...
class Object:
    ...
    def move(self, dx, dy):
        ...
        if not mapgen.is_blocked(self.x + dx, self.y + dy):
            self.x += dx
            self.y += dy

    def moveai(self, dx, dy):
        if not mapgen.is_blocked(self.x, self.y + dy):
            self.y += dy
        if not mapgen.is_blocked(self.x + dx, self.y):
            self.x += dx
...
class Equipment:
    #object that can be equipped with bonuses. Automatically adds the Item component.
    ...
    def toggle_equip(self):  # toggle equip/dequip status of an item
        if self.is_equipped:
            self.dequip()
        else:
            self.equip()
...
def make_map():
    ...
    for r in range(MAX_ROOMS):
        ...
        if not failed:
            ...
            mapgen.create_room(new_room)

            #add some contents to this room, such as monsters
            mapgen.place_objects(new_room)
            ...
            if num_rooms == 0:
                ...
                if libtcod.random_get_int(0, 0, 1) == 1:
                    ...
                    mapgen.create_h_tunnel(prev_x, new_x, prev_y)
                    mapgen.create_v_tunnel(prev_y, new_y, new_x)
                else:
                    #first move vertically, then horizontally
                    mapgen.create_v_tunnel(prev_y, new_y, prev_x)
                    mapgen.create_h_tunnel(prev_x, new_x, new_y)
...
def move_camera(target_x, target_y):
    ...
    if x < 0: x = 0
    if y < 0: y = 0
    if x > mapgen.MAP_WIDTH - CAMERA_WIDTH - 1: x = mapgen.MAP_WIDTH - CAMERA_WIDTH - 1
    if y > mapgen.MAP_HEIGHT - CAMERA_HEIGHT - 1: y = mapgen.MAP_HEIGHT - CAMERA_HEIGHT - 1
...
#
#     rollcounter += 2
#     message(str(rollcounter), libtcod.white)
#
#
...
# def cast_dizziness():
#     #make player dizzy
#     if player.fighter.dizzy >= DIZZINESS_TRIGGER:
...
#
#     #target!
#     message('ATTACK hits ' + monster.name + ' and deals '
...
def next_level():
    ...
    message('You sit in the stairwell for a moment to recover your strength', libtcod.light_violet)
